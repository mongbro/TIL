...
class Sprite:
    ...
    # 다른 스프라이트와의 충돌 여부를 계산 => 충돌하면 true 반환
    # 3장의 Lab 문제 참조
    ...
    # 충돌을 처리한다. 현재는 아무일도 하지 않는다. 자식 클래스에서 오버라이드
    def handleCollision(self, other):
        pass


# 아군 우주선을 나타내는 클래스
class StarShopSprite(Sprite):
    ...
    # 우주선을 움직인다. 만약 윈도우의 경계선을 넘으려고 하면 움직이지 않는다.
    def move(self):
...
class AlienSprite(Sprite):
    def __init__(self, game, image, x, y):
        super().__init(image, x, y)
        self.game = game
        self.dx = -10  # x방향으로만 움직인다.

    # 외계인 우주선을 움직이는 메소드
    # 윈도우 경계에 도달하면 한칸 아래로 이동한다.
    def move(self):
...
class ShotSprite(Sprite):
    ...
    # 충돌을 처리한다. 미사일과 외계인 우주선 객체를 모두 리스트에서 삭제한다.
    def handleCollision(self, other):
        if type(other) is AlienSprite:
            self.game.removeSprite(self)
...
class GalagaGame():
    ...
    def __init__(self, master):
        ...
        self.canvas.pack()
        self.shotImage = PhotoImage(file = "C:\Users\Mong_Bro\source\repos\mongbro\TIL\PYTHON\Gallag\missile.png")
        self.shipImage = PhotoImage(file = "C:\Users\Mong_Bro\source\repos\mongbro\TIL\PYTHON\Gallag\StarShip.png")
        self.alienImage = PhotoImage(file = "C:\Users\Mong_Bro\source\repos\mongbro\TIL\PYTHON\Gallag\Alien.png")
        self.running = True
        self.initSprites()
        master.bind("<Left>", self.keyLeft())
        master.bind("<Right>", self.keyRight())
        master.bing("<space>", self.keySpace())
    ...
    # 미사일을 발사하는 메소드
    def fire(self):
        shot = ShotSprite(self, self.shipImage, self.starship.getx() + 10, self.starship.getY() - 30)
        self.sprites.append(shot)

    # 화면을 그리는 메소드
    def paint(self, g):
        #self.canvas.delete(All)
        self.canvas.create_rectangle(0,0,800,600,file = "C:\Users\Mong_Bro\source\repos\mongbro\TIL\PYTHON\Gallag\background")
